...
The Id of each tab will be read accordingly as: `<namespace>:<category_path>`
...
│   │   └── <criterion_key> <- String (Identifier)
...
- **`prerequisites`**: (TAG_Compound) *(Optional)* Prerequisites required to view and unlock the entire category tab. If not specified, defaults to always met.
  - **`advancements`**: (TAG_Compound) *(Optional)* Maps arbitrary criterion string keys to Advancement Identifiers (e.g., `"minecraft:story/mine_stone"`).
  - **`parents`**: (TAG_Compound) *(Optional)* Maps arbitrary criterion string keys to parent Skill Category Identifiers.
...
The Id of each node will be read accordingly as: `<namespace>:<category_path>/<node_name>`
...
- **`layout`**: (TAG_Compound) Layout graph structure configuration.
  ...
  - **`prerequisites`**: (TAG_Compound) *(Optional)* Prerequisites for when to display this node in player's GUI. If not specified, this node is always visible. \*
    - **`advancements`**: (TAG_Compound) *(Optional)* Maps arbitrary criterion string keys to Advancement Identifiers.
...
- **`advancements`**: A map of custom criterion keys to Minecraft Advancement Identifiers (`<namespace>:<path>`).
- **`parents`**: A map of custom criterion keys to parent Skill Nodes (or parent Skill Categories) Identifiers.

...
public interface WeatheringItem<T extends Enum<T>> {
    ...
    static boolean checkConditionsToWeatherExtra(ItemStack thisItem, ItemStack[] surroundingItems, RandomSource random) {
        if (isRegisteredExtraWeatheringItem(thisItem.getItem())) {
            if (random.nextFloat() < getExtraOxidationChance(thisItem.getItem())) {
    ...
    record CompositeWeatherstateOxidation(int[] weatherStateOrdinal, float oxidationChanceDirect, Class<?> classWeatheringItem) {}
    ...
    public static void registerExtraWeatheringItem(Item a, int[] b, float c, Class<?> d) {
        EXTRA_WEATHERING_ITEM.put(a, new CompositeWeatherstateOxidation(b,c,d));
    }
    ...
    default boolean checkWeatheringSurroundings(ItemStack thisItem, ItemStack[] surroundingItems, RandomSource random) {
        ...
        Class<?> this_enum_class = this.getAge().getClass();
        float chance_surrounding = calculateChanceBasedSurrounding(thisItem, this_age, this_enum_class, surroundingItems, random);
    ...
    static boolean checkWeatheringSurroundingsExtra(ItemStack thisItem, ItemStack[] surroundingItems, RandomSource random) {
        ...
        Class<?> this_enum_class = getExtraClass(thisItem.getItem());
        float chance_surrounding = calculateChanceBasedSurrounding(thisItem, this_age, this_enum_class, surroundingItems, random);
    ...
    public static boolean isValidToWeather(Item item) {
        if (item instanceof WeatheringItem weatheringItem) {
            ...
            if (isRegisteredExtraWeatheringItem(item) && !item.equals(Items.AIR)) {

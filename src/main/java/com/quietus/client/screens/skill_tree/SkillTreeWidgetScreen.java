...
public class SkillTreeWidgetScreen implements SkillTreeDraggable, ClientSkillTreeListener {
    ...
    private static final Identifier CONTAINER_CONTENTS_SPRITE_LOCATION = Identifier.fromNamespaceAndPath(MODID, "skill_tree/container_contents");
    private static final Identifier CONTAINER_HEADER_SPRITE_LOCATION = Identifier.fromNamespaceAndPath(MODID, "skill_tree/container_header");
    private static final Identifier UPGRADE_BUTTON_SPRITE_LOCATION = Identifier.fromNamespaceAndPath(MODID, "skill_tree/upgrade_button");
    private static final Identifier CLOSE_BUTTON_SPRITE_LOCATION = Identifier.fromNamespaceAndPath(MODID, "skill_tree/cross_button");
    private static final Identifier CLOSE_BUTTON_HIGHLIGHTED_SPRITE_LOCATION = Identifier.fromNamespaceAndPath(MODID, "skill_tree/cross_button_highlighted");
    ...
    public void draw(GuiGraphicsExtractor GuiGraphicsExtractor, int mouseX, int mouseY, int offsetX, int offsetY) {
        ...
        // contents container
        GuiGraphicsExtractor.blitSprite(RenderPipelines.GUI_TEXTURED, CONTAINER_CONTENTS_SPRITE_LOCATION, window_x, window_y, WIDTH, this.height);
        // header container
        GuiGraphicsExtractor.blitSprite(RenderPipelines.GUI_TEXTURED, CONTAINER_HEADER_SPRITE_LOCATION, window_x, window_y, WIDTH, this.headerTextHeight + V_MARGIN + SECTION_V_MARGIN + SkillTreeWidget.ICON_HEIGHT + HEADER_CONTENTS_PADDING);
        // header
        GuiGraphicsExtractor.textWithWordWrap(font, header, render_x, render_y - SECTION_V_MARGIN - this.headerTextHeight, WIDTH, 0xFFFFFFFF, true);
        // description
        GuiGraphicsExtractor.textWithWordWrap(font, description, render_x + CONTENT_H_MARGIN, render_y + SkillTreeWidget.ICON_HEIGHT + HEADER_CONTENTS_PADDING*2, WIDTH, 0xFFFFFFFF, true);
        // unlock button
        if (this.isMaxed()) {
            
        } else {
            GuiGraphicsExtractor.blitSprite(RenderPipelines.GUI_TEXTURED, UPGRADE_BUTTON_SPRITE_LOCATION, render_x + SECTION_H_MARGIN + SkillTreeWidget.ICON_WIDTH, render_y+4, UPGRADE_BUTTON_WIDTH, UPGRADE_BUTTON_HEIGHT);
            //GuiGraphicsExtractor.fill(render_x + SECTION_H_MARGIN + SkillTreeWidget.ICON_WIDTH, render_y+4, render_x + SECTION_H_MARGIN + SkillTreeWidget.ICON_WIDTH + 134, render_y+18+4, 0xFF00BB20);
            if (this.amount == 0) 
                GuiGraphicsExtractor.centeredText(font, Component.translatable("skillTree.quietus.unlock", this.amount, this.maxAmount), render_x + SECTION_H_MARGIN + SkillTreeWidget.ICON_WIDTH + 67, render_y+4+4, 0xFFFFFFFF);
            if (this.amount > 0) 
                GuiGraphicsExtractor.centeredText(font, Component.translatable("skillTree.quietus.upgrade", this.amount, this.maxAmount), render_x + SECTION_H_MARGIN + SkillTreeWidget.ICON_WIDTH + 67, render_y+4+4, 0xFFFFFFFF);
        }
        // icon (drawn again in addition to the widget drawing itself in the tab)
        this.widget.drawAbsolute(GuiGraphicsExtractor, render_x, render_y);
        // close button
        if (this.isMouseOverCloseButton(offsetX, offsetY, mouseX, mouseY)) 
            GuiGraphicsExtractor.blitSprite(RenderPipelines.GUI_TEXTURED, CLOSE_BUTTON_HIGHLIGHTED_SPRITE_LOCATION, window_x + WIDTH - H_MARGIN - CLOSE_BUTTON_WIDTH, window_y + V_MARGIN, CLOSE_BUTTON_WIDTH, CLOSE_BUTTON_HEIGHT);  
        else 
            GuiGraphicsExtractor.blitSprite(RenderPipelines.GUI_TEXTURED, CLOSE_BUTTON_SPRITE_LOCATION, window_x + WIDTH - H_MARGIN - CLOSE_BUTTON_WIDTH, window_y + V_MARGIN, CLOSE_BUTTON_WIDTH, CLOSE_BUTTON_HEIGHT);
    }
